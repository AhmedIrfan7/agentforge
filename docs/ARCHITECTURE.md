...
_Milestone 11 (steps 267–280) is complete — this section documents it in full, step by step._
...
Fourteen steps, and — unlike Milestone 10 — every single one is real infrastructure this project genuinely didn't have before, not an audit of already-correct behavior: two Dockerfiles that existed but had zero CI coverage until step 267 gave them real build+smoke-test enforcement; a single-host reference deployment (`docker-compose.prod.yml`) and the documentation walking through actually running it; CI/CD that really publishes images to GHCR and really deploys via SSH to a staging/production target (honestly waiting on secrets that don't exist yet, but genuinely correct and ready the moment they do); a real `/ready` endpoint distinct from `/health`; horizontal scaling and CDN hardening that needed no redesign, only configuration and live-verified confirmation that the underlying mechanisms actually work; every real environment variable indexed in one place; a rollback procedure proven against a real database, not assumed from the existence of an `alembic downgrade` command; a real Terraform skeleton for one concrete cloud target; and a real, working uptime/alerting loop, built from GitHub Actions itself rather than a third-party account this project doesn't have.

The same throughline Milestone 10 established holds here too, tested harder: live verification over assumption, in an area — deployment infrastructure — where the surface area for `terraform apply`-but-nothing-happens or `docker compose up`-but-it-crashes is much larger than in application code. Every step that could be run for real, was: a real Docker image built and smoke-tested against zero external services, a real backup restored into a real throwaway container, a real migration rolled back and forward again, a real SRI hash independently cross-checked with `openssl`, a real GitHub issue opened and closed by a real workflow run. Several of those runs caught real bugs that reading the code alone would have missed entirely (a broken worker startup command, a Next.js build-time-vs-runtime env var, an old-format migration a script would have silently mis-parsed) — the same lesson Milestone 10 closed on, borne out again on harder ground.
