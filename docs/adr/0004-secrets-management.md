...
**The chosen design:** `config.py:Settings._reject_placeholder_secrets_in_production` raises at startup, not silently in production, if `SECRET_KEY`, `JWT_SECRET`, `MFA_ENCRYPTION_KEY`, or `STORAGE_SECRET_KEY` still hold their obvious placeholder values when `ENVIRONMENT=production`. Deliberately scoped to exactly these four: each is *directly* usable to compromise the system (or, for `STORAGE_SECRET_KEY`, the object store) the instant its real value is known — AGENTS.md §9's "insecure defaults are a design bug, not an ops footnote" applies most sharply here.

**`STORAGE_SECRET_KEY` was a real, undated gap until roadmap step 299's final documentation review closed it:** its local-dev default (`agentforge123`) is a real, guessable credential, and unlike the other three it was never checked against a placeholder list — a production deployment that forgot to override it would have silently used a guessable object-storage credential. `GOOGLE_CLIENT_SECRET`, `OPENAI_API_KEY`, and `ANTHROPIC_API_KEY` remain deliberately NOT covered by this guard: left empty in production, they only make their own feature fail closed (Google login rejects the request; an LLM call raises `LLMProviderError`) rather than exposing anything — an intentional, already-documented exception (`config.py`'s own comment on `google_client_secret`), not an oversight.
